...
# Support vertical segments, (the bentley-ottmann method doesn't support
# this). We use the term 'START_VERTICAL' for a vertical segment,
...
class Event:
    ...
    def y_intercept_x(self, x: float):
        ...
        if x >= self.segment[1][X]:
            return self.segment[1][Y]

        # Use the largest to avoid float precision error with nearly
        # vertical lines.
    ...
    def Compare(sweep_line, this, that):
        ...
        # otherwise w/ float precision errors we get incorrect
        # comparisons can get very strange & hard to debug output
        # without this.
        if abs(delta_y) > EPS:
...
class SweepLine:
    __slots__ = (
        ...
        # if an event is in the tree will return true if its opposite
        # side is found.
        # This is essential for the algorithm to work, and why we don't
        # explicitly remove START events. Instead, the END events are
        # never added to the current sweep, and removing them also
        # removes the start.
    ...
    def insert(self, event):
        ...
        if USE_DEBUG:
            assert not event.in_sweep
            assert not event.other.in_sweep
    ...
    def handle_event(self, event):
        ...
        if t == Event.Type.START:
            ...
            # In this case we only need to find all segments in this
            # span.
...
class EventQueue:
    ...
    def __init__(self, segments, line: SweepLine):
        ...
        # segments = [s for s in segments if s[0][0] != s[1][0] and
        # s[0][1] != s[1][1]]
...
class _ABCTree:
    ...
    def succ_item(self, key, default=_sentinel):
        ...
        # faster with pypy than the Cython version
        # Note: Code sharing of succ_item() and ceiling_item() is
        # possible, but has always a speed penalty.
        node = self._root
        succ_node = None
        while node is not None:
            cmp = self._cmp(self._cmp_data, key, node.key)
    ...
    def prev_item(self, key, default=_sentinel):
        ...
        # CPython and much slower on pypy. This version runs about 4x
        # faster with pypy than the Cython version
        # Note: Code sharing of prev_item() and floor_item() is
        # possible, but has always a speed penalty.
        node = self._root
        prev_node = None
